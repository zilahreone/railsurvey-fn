...
<template>
  <div>
    <div class="relative">
      <input @input="emit('update:modelValue', ($event.target.value))" :value="modelValue" :type="type" class="_input" :placeholder="placeholder">
      <button @click="isActive = !isActive" class="absolute right-8 bottom-2.5 text-sm text-gray-500">
        <svg class="w-4 h-4" aria-hidden="true" fill="none" stroke="currentColor"
          viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
        </svg>
      </button>
      <div v-show="isActive" id="dropdown" class="absolute bg-white border shadow w-full dark:bg-gray-700">
